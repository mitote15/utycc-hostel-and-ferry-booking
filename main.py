...
def get_db_connection():
  ...
  if os.path.exists(TMP_DB_FILE):
    try:
      conn = sqlite3.connect(TMP_DB_FILE, timeout=15)
      return conn
    except Exception:
      pass
...
def verify_user(student_id, password):
  ...
  cursor = conn.cursor()
  cursor.execute(
      "SELECT id, name, student_id, email, phone, role FROM users WHERE"
      " TRIM(student_id)=? AND password=?",
      (str(student_id).strip(), str(password)),
  )
  row = cursor.fetchone()
  conn.close()
  if row:
    u_id, u_name, u_s_id, u_email, u_phone, u_role = row
    return {
        "id": u_id,
        "name": u_name,
        "student_id": u_s_id,
...
def register_user(name, student_id, email, phone, password):
  ...
  cursor.execute(
      "SELECT id FROM users WHERE TRIM(student_id)=?", (clean_student_id,)
  )
  if cursor.fetchone():
    conn.close()
...
def verify_admin(username, password, role=None):
  ...
  cursor = conn.cursor()
  cursor.execute(
      "SELECT id, username, name, email, role FROM admins WHERE username=? AND"
      " password=?",
      (username, password),
  )
  row = cursor.fetchone()
  ...
  if row:
    ...
    if role and str(role).strip():
      ...
      if req_role == "super_admin" and db_role != "super_admin":
        return None
      if (
          req_role in ["hostel", "ferry"]
          and db_role != req_role
          and db_role != "super_admin"
      ):
        return None
    return {
        "id": a_id,
...
class UTYCCPortalHandler(SimpleHTTPRequestHandler):
  ...
  def do_GET(self):
    ...
    if path in ["/", "/index.html", "/home", "/home.html"]:
      ...
      return
    elif path == "/api/admin/overview" or path.startswith(
        "/api/admin/overview"
    ):
  ...
  def do_POST(self):
    ...
    if path in ["/api/user/update-profile", "/api/update-profile"]:
  ...
  def handle_update_profile(self, data):
    ...
    if not student_id or not name:
      self.send_json(
          {"success": False, "message": "Student ID and Name are required."}, 400
      )
      return
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE users SET name=?, email=?, phone=?, avatar=? WHERE"
        " TRIM(student_id)=?",
  ...
  def handle_change_password(self, data):
      ...
      self.send_json(
          {
              "success": False,
              "message": (
                  "Student ID, current password, and new password are required."
              ),
          },
          400,
      )
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id FROM users WHERE TRIM(student_id) = ? AND password = ?",
    ...
    if not user_row:
      conn.close()
      self.send_json(
          {"success": False, "message": "လက်ရှိ စကားဝှက် မှားယွင်းနေပါသည်။"},
          400,
      )
      return

    cursor.execute(
        "UPDATE users SET password = ? WHERE TRIM(student_id) = ?",
  ...
  def handle_admin_register(self, data):
      ...
      self.send_json(
          {
              "success": False,
              "message": (
                  "Public registration for Super Admin role is restricted."
              ),
          },
          403,
      )
    ...
    if cursor.fetchone():
      conn.close()
      self.send_json(
          {"success": False, "message": "Admin username already registered."},
          400,
      )
      return
    ...
    cursor.execute(
        ...
        (
            ...
            password,
            name or username,
            email or f"{username}@utycc.edu.mm",
            role,
        ),
    )
    conn.commit()
    ...
    self.send_json({
        "success": True,
        "message": (
            f"{role.upper()} Admin account registered successfully! You can now"
            " log in."
        ),
    })
  ...
  def handle_create_admin(self, data):
    ...
    self.send_json({
        "success": True,
        "message": f"New {role.upper()} account created successfully!",
    })
  ...
  def handle_add_hostel(self, data):
    ...
    image_url = (
        ...
        or "https://images.unsplash.com/photo-1555854877-bab0e564b8d5?auto=format&fit=crop&w=800&q=80"
    )
    ...
    if not hostel_name or not room_number:
      self.send_json(
          {
              "success": False,
              "message": "Hostel Name and Room Number are required.",
          },
          400,
      )
      return
    ...
    self.send_json({
        "success": True,
        "message": "Hostel block added successfully with custom KPay scanner!",
    })
  ...
  def handle_edit_hostel(self, data):
    ...
    if not hostel_id or not hostel_name or not room_number:
      self.send_json(
          {
              "success": False,
              "message": "Hostel ID, Name, and Room Number are required.",
          },
          400,
      )
      return
    ...
    if admin_id:
      cursor.execute(
          """
                UPDATE hostels 
                SET hostel_name=?, room_number=?, gender_type=?, capacity=?, available_beds=?, description=?, image_url=?, kpay_qr_url=?, status=?
                WHERE hostel_id=? AND (admin_id=? OR admin_id IS NULL)
            """,
          (
              hostel_name,
              room_number,
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Hostel information updated successfully!"}
    )
  ...
  def handle_toggle_hostel_status(self, data):
    ...
    cursor.execute(
        "SELECT status FROM hostels WHERE hostel_id = ?", (hostel_id,)
    )
    row = cursor.fetchone()
    if not row:
      conn.close()
    ...
    if admin_id:
      cursor.execute(
          "UPDATE hostels SET status = ? WHERE hostel_id = ? AND (admin_id = ?"
          " OR admin_id IS NULL)",
          (new_status, hostel_id, admin_id),
      )
    else:
      cursor.execute(
          "UPDATE hostels SET status = ? WHERE hostel_id = ?",
          (new_status, hostel_id),
      )
    ...
    self.send_json({
        "success": True,
        "message": f"Hostel status set to {new_status}",
        "status": new_status,
    })
  ...
  def handle_delete_hostel(self, data):
    ...
    if admin_id:
      cursor.execute(
          "DELETE FROM hostels WHERE hostel_id = ? AND (admin_id = ? OR"
          " admin_id IS NULL)",
          (hostel_id, admin_id),
      )
    else:
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Hostel block removed successfully!"}
    )
  ...
  def handle_add_ferry_group(self, data):
    ...
    if not group_name:
      self.send_json(
          {"success": False, "message": "Group Name is required."}, 400
      )
      return
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Ferry route group created successfully!"}
    )
  ...
  def handle_delete_ferry_group(self, data):
    ...
    if admin_id:
      cursor.execute(
          "DELETE FROM ferry_groups WHERE group_id = ? AND (admin_id = ? OR"
          " admin_id IS NULL)",
          (group_id, admin_id),
      )
    else:
      cursor.execute(
          "DELETE FROM ferry_groups WHERE group_id = ?", (group_id,)
      )

    conn.commit()
    conn.close()
    self.send_json(
        {"success": True, "message": "Ferry group removed successfully!"}
    )
  ...
  def handle_add_ferry(self, data):
    ...
    if not ferry_name or not route_name:
      self.send_json(
          {
              "success": False,
              "message": "Ferry Name and Route Name are required.",
          },
          400,
      )
      return
    ...
    self.send_json({
        "success": True,
        "message": "Ferry line added successfully with custom KPay scanner!",
    })
  ...
  def handle_edit_ferry(self, data):
    ...
    if not ferry_id or not ferry_name or not route_name:
      self.send_json(
          {
              "success": False,
              "message": "Ferry ID, Name, and Route Name are required.",
          },
          400,
      )
      return
    ...
    if admin_id:
      cursor.execute(
          """
                UPDATE ferries 
                SET group_id=?, ferry_name=?, route_name=?, departure_time=?, capacity=?, available_seats=?, kpay_qr_url=?, status=?
                WHERE ferry_id=? AND (admin_id=? OR admin_id IS NULL)
            """,
          (
              group_id,
              ferry_name,
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Ferry route updated successfully!"}
    )
  ...
  def handle_toggle_ferry_status(self, data):
    ...
    cursor.execute(
        "SELECT status FROM ferries WHERE ferry_id = ?", (ferry_id,)
    )
    row = cursor.fetchone()
    if not row:
      conn.close()
    ...
    if admin_id:
      cursor.execute(
          "UPDATE ferries SET status = ? WHERE ferry_id = ? AND (admin_id = ?"
          " OR admin_id IS NULL)",
          (new_status, ferry_id, admin_id),
      )
    else:
      cursor.execute(
          "UPDATE ferries SET status = ? WHERE ferry_id = ?",
          (new_status, ferry_id),
      )
    ...
    self.send_json({
        "success": True,
        "message": f"Ferry status set to {new_status}",
        "status": new_status,
    })
  ...
  def handle_delete_ferry(self, data):
    ...
    if admin_id:
      cursor.execute(
          "DELETE FROM ferries WHERE ferry_id = ? AND (admin_id = ? OR admin_id"
          " IS NULL)",
          (ferry_id, admin_id),
      )
    else:
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Ferry line removed successfully!"}
    )
  ...
  def handle_get_ferries(self):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT ferry_id, group_id, ferry_name, route_name, departure_time,"
        " capacity, available_seats, status, kpay_qr_url FROM ferries"
    )
  ...
  def handle_get_ferry_groups(self):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT group_id, group_name, description FROM ferry_groups"
    )
    rows = cursor.fetchall()
    conn.close()
    groups = []
    for gid, gname, desc in rows:
      groups.append(
          {"group_id": gid, "group_name": gname, "description": desc}
      )
  ...
  def handle_get_ferries_by_group(self, group_id):
    ...
    cursor.execute(
        "SELECT ferry_id, group_id, ferry_name, route_name, departure_time,"
        " capacity, available_seats, status, kpay_qr_url FROM ferries WHERE"
        " group_id = ?",
        (group_id,),
    )
    rows = cursor.fetchall()
  ...
  def handle_get_hostels(self):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT hostel_id, hostel_name, room_number, gender_type, capacity,"
        " available_beds, description, image_url, status, kpay_qr_url FROM"
        " hostels"
    )
    rows = cursor.fetchall()
    conn.close()
  ...
  def handle_user_status(self, student_id):
    ...
    cursor.execute(
        "SELECT hb.booking_id, h.hostel_id, h.hostel_name, h.room_number,"
        " hb.payment_status, hb.status FROM hostel_bookings hb JOIN hostels h ON"
        " hb.hostel_id = h.hostel_id WHERE TRIM(hb.student_id) = ? AND"
        " hb.status != 'Cancelled' ORDER BY hb.booking_id DESC LIMIT 1",
        (str(student_id).strip(),),
    )
    hostel_row = cursor.fetchone()

    cursor.execute(
        "SELECT fb.booking_id, f.ferry_id, f.ferry_name, f.route_name,"
        " fb.payment_status, fb.status FROM ferry_bookings fb JOIN ferries f ON"
        " fb.ferry_id = f.ferry_id WHERE TRIM(fb.student_id) = ? AND fb.status"
        " != 'Cancelled' ORDER BY fb.booking_id DESC LIMIT 1",
        (str(student_id).strip(),),
    )
    ...
    hostel_data = None
    if hostel_row:
      h_bid, h_hid, h_hname, h_rnum, h_payst, h_st = hostel_row
      hostel_data = {
          "booking_id": h_bid,
          "hostel_id": h_hid,
          "hostel_name": h_hname,
    ...
    ferry_data = None
    if ferry_row:
      f_bid, f_fid, f_fname, f_rname, f_payst, f_st = ferry_row
      ferry_data = {
          "booking_id": f_bid,
          "ferry_id": f_fid,
          "ferry_name": f_fname,
  ...
  def handle_get_reviews(self):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT review_id, name, email, rating, review_text, created_at FROM"
        " reviews ORDER BY review_id DESC"
    )
    rows = cursor.fetchall()
    conn.close()
    reviews = []
    ...
    avg_rating = (
        round(total_rating_sum / total_count, 1) if total_count > 0 else 0.0
    )
  ...
  def handle_user_login(self, data):
    ...
    if user:
      ...
      self.send_json(
          {"success": False, "message": "Invalid Student ID or password"}, 401
      )
  ...
  def handle_admin_login(self, data):
    ...
    if admin:
      ...
      self.send_json(
          {
              "success": False,
              "message": "Invalid admin credentials or unauthorized role access",
          },
          401,
      )
  ...
  def handle_hostel_book(self, data):
    ...
    if not student_id:
      self.send_json(
          {"success": False, "message": "Student Roll ID is required."}, 400
      )
      return
    ...
    cursor.execute(
        "SELECT booking_id FROM hostel_bookings WHERE TRIM(student_id) = ? AND"
        " status IN ('Pending Approval', 'Allocated')",
        (student_id,),
    )
    if cursor.fetchone():
      conn.close()
      self.send_json(
          {
              "success": False,
              "message": (
                  "သင်သည် Hostel Booking ပြုလုပ်ထားပြီးဖြစ်သည်။ အကောင့်တစ်ခုလျှင်"
                  " Booking တစ်ခုသာ တင်ခွင့်ရှိပါသည်။"
              ),
          },
          400,
      )
    ...
    cursor.execute(
        "UPDATE hostels SET available_beds = MAX(0, available_beds - 1) WHERE"
        " hostel_id = ?",
        (data.get("hostel_id", 1),),
    )
    conn.commit()
    conn.close()
    self.send_json(
        {"success": True, "message": "Hostel booking allocated successfully!"}
    )
  ...
  def handle_ferry_book(self, data):
    ...
    cursor.execute(
        "SELECT booking_id FROM ferry_bookings WHERE TRIM(student_id) = ? AND"
      ...
      self.send_json(
          {
              "success": False,
              "message": (
                  "သင်သည် Ferry Ticket ရယူထားပြီးဖြစ်သည်။ အကောင့်တစ်ခုလျှင် Booking"
                  " တစ်ခုသာ တင်ခွင့်ရှိပါသည်။"
              ),
          },
          400,
      )
    ...
    cursor.execute(
        "UPDATE ferries SET available_seats = MAX(0, available_seats - 1)"
        " WHERE ferry_id = ?",
        (data.get("ferry_id", 1),),
    )
    conn.commit()
    conn.close()
    self.send_json(
        {"success": True, "message": "Ferry ticket reserved successfully!"}
    )
  ...
  def handle_inquiry(self, data):
    ...
    cursor.execute(
        "INSERT INTO inquiries (full_name, student_id, service_category,"
        " message) VALUES (?, ?, ?, ?)",
        (
            data.get("full_name", ""),
            data.get("student_id", ""),
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Support inquiry submitted successfully!"}
    )
  ...
  def handle_cancel_hostel(self, data):
    ...
    if not student_id:
      self.send_json(
          {"success": False, "message": "Student ID is required."}, 400
      )
      return
    ...
    cursor.execute(
        "SELECT hostel_id FROM hostel_bookings WHERE TRIM(student_id) = ? AND"
        " status != 'Cancelled' ORDER BY booking_id DESC LIMIT 1",
        (student_id,),
    )
    row = cursor.fetchone()
    if row:
      hostel_id = row[0]
      cursor.execute(
          "DELETE FROM hostel_bookings WHERE TRIM(student_id) = ?",
          (student_id,),
      )
      cursor.execute(
          "UPDATE hostels SET available_beds = available_beds + 1 WHERE"
          " hostel_id = ?",
          (hostel_id,),
      )
      conn.commit()
      conn.close()
      self.send_json(
          {"success": True, "message": "Hostel booking cancelled successfully!"}
      )
    else:
      conn.close()
      self.send_json(
          {"success": False, "message": "No active booking found"}, 400
      )
  ...
  def handle_cancel_ferry(self, data):
    ...
    cursor.execute(
        "SELECT ferry_id FROM ferry_bookings WHERE TRIM(student_id) = ? AND"
        " status != 'Cancelled' ORDER BY booking_id DESC LIMIT 1",
        (student_id,),
    )
    row = cursor.fetchone()
    if row:
      ferry_id = row[0]
      cursor.execute(
          "DELETE FROM ferry_bookings WHERE TRIM(student_id) = ?",
          (student_id,),
      )
      cursor.execute(
          "UPDATE ferries SET available_seats = available_seats + 1 WHERE"
          " ferry_id = ?",
          (ferry_id,),
      )
      conn.commit()
      conn.close()
      self.send_json(
          {"success": True, "message": "Ferry booking cancelled successfully!"}
      )
    else:
      conn.close()
      self.send_json(
          {"success": False, "message": "No active ticket found"}, 400
      )
  ...
  def handle_hostel_update_booking(self, data):
    ...
    if not booking_id:
      self.send_json(
          {"success": False, "message": "Booking ID is required."}, 400
      )
      return
    ...
    cursor.execute(
        """
            ...
            SET hostel_id=?, student_name=?, student_year=?, national_id=?, transaction_id=?
            WHERE booking_id=? AND TRIM(student_id)=?
        """,
        (
            hostel_id,
            student_name,
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Hostel booking updated successfully!"}
    )
  ...
  def handle_ferry_update_booking(self, data):
    ...
    cursor.execute(
        """
            UPDATE ferry_bookings 
            SET ferry_id=?, student_name=?, student_year=?, student_phone=?, transaction_id=?
            WHERE booking_id=? AND TRIM(student_id)=?
        """,
        (
            ferry_id,
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": "Ferry ticket updated successfully!"}
    )
  ...
  def handle_admin_overview(self, role="", admin_id=None):
    ...
    try:
      admin_id_int = (
          int(admin_id)
          if admin_id is not None and str(admin_id).isdigit()
          else None
      )
    except (ValueError, TypeError):
      admin_id_int = None
    ...
    if is_super:
      cursor.execute(
          "SELECT id, username, name, email, role, created_at FROM admins"
      )
    ...
    if is_super:
      cursor.execute(
          "SELECT hostel_id, hostel_name, room_number, gender_type,"
          " available_beds, capacity, description, image_url, kpay_qr_url,"
          " status, admin_id FROM hostels"
      )
      for (
          hid,
          hname,
      ...
      if admin_id_int:
        cursor.execute(
            "SELECT hostel_id, hostel_name, room_number, gender_type,"
            " available_beds, capacity, description, image_url, kpay_qr_url,"
            " status, admin_id FROM hostels WHERE admin_id = ?",
            (admin_id_int,),
        )
      else:
        cursor.execute(
    ...
    if is_super:
      cursor.execute(
          "SELECT group_id, group_name, description, admin_id FROM"
          " ferry_groups"
      )
      ...
      if admin_id_int:
        cursor.execute(
            "SELECT group_id, group_name, description, admin_id FROM"
            " ferry_groups WHERE admin_id = ? OR admin_id IS NULL",
            (admin_id_int,),
        )
      else:
        cursor.execute(
    ...
    if is_super:
      cursor.execute(
          "SELECT ferry_id, group_id, ferry_name, route_name, departure_time,"
          " available_seats, capacity, kpay_qr_url, status, admin_id FROM"
          " ferries"
      )
      for (
          fid,
    ...
    if is_super:
      cursor.execute(
          "SELECT id, name, student_id, email, phone, role, created_at FROM"
          " users"
      )
    ...
    if is_super:
      cursor.execute(
          "SELECT hb.booking_id, hb.hostel_id, hb.student_name,"
          " hb.student_year, hb.student_id, hb.national_id, hb.payment_method,"
          " hb.transaction_id, hb.amount, hb.payment_status, hb.booking_date,"
          " hb.status, h.hostel_name, h.room_number FROM hostel_bookings hb"
          " JOIN hostels h ON hb.hostel_id = h.hostel_id ORDER BY"
          " hb.booking_id DESC"
      )
      for (
          bid,
      ...
      if admin_id_int:
        cursor.execute(
            "SELECT hb.booking_id, hb.hostel_id, hb.student_name,"
            " hb.student_year, hb.student_id, hb.national_id,"
            " hb.payment_method, hb.transaction_id, hb.amount,"
            " hb.payment_status, hb.booking_date, hb.status, h.hostel_name,"
            " h.room_number FROM hostel_bookings hb JOIN hostels h ON"
            " hb.hostel_id = h.hostel_id WHERE h.admin_id = ? ORDER BY"
            " hb.booking_id DESC",
            (admin_id_int,),
        )
      else:
        cursor.execute(
            ...
            " hb.payment_status, hb.booking_date, hb.status, h.hostel_name,"
            " h.room_number FROM hostel_bookings hb JOIN hostels h ON"
            " hb.hostel_id = h.hostel_id ORDER BY hb.booking_id DESC"
        )
      for (
          bid,
          hid,
    ...
    if is_super:
      cursor.execute(
          "SELECT fb.booking_id, fb.ferry_id, fb.student_name, fb.student_year,"
          " fb.student_id, fb.student_phone, fb.payment_method,"
          " fb.transaction_id, fb.amount, fb.payment_status, fb.booking_date,"
          " fb.status, f.ferry_name, f.route_name FROM ferry_bookings fb JOIN"
          " ferries f ON fb.ferry_id = f.ferry_id ORDER BY fb.booking_id DESC"
      )
      for (
          bid,
          fid,
      ...
      if admin_id_int:
        cursor.execute(
            "SELECT fb.booking_id, fb.ferry_id, fb.student_name,"
            " fb.student_year, fb.student_id, fb.student_phone,"
            " fb.payment_method, fb.transaction_id, fb.amount,"
            " fb.payment_status, fb.booking_date, fb.status, f.ferry_name,"
            " f.route_name FROM ferry_bookings fb JOIN ferries f ON fb.ferry_id"
            " = f.ferry_id WHERE f.admin_id = ? ORDER BY fb.booking_id DESC",
            (admin_id_int,),
        )
      else:
        cursor.execute(
            ...
            " fb.payment_status, fb.booking_date, fb.status, f.ferry_name,"
            " f.route_name FROM ferry_bookings fb JOIN ferries f ON fb.ferry_id"
            " = f.ferry_id ORDER BY fb.booking_id DESC"
        )
      for (
          bid,
          fid,
    ...
    if is_super or is_hostel:
      pending_payments += [
          ...
          if hb["payment_status"] == "Pending"
          or hb["status"] == "Pending Approval"
      ]
    if is_super or is_ferry:
      pending_payments += [
          fb
          for fb in ferry_bookings
          if fb["payment_status"] == "Pending"
          or fb["status"] == "Pending Approval"
      ]

    inquiries = []
    if is_super:
      cursor.execute(
          "SELECT inquiry_id, full_name, student_id, service_category, message,"
          " status, created_at FROM inquiries ORDER BY inquiry_id DESC"
      )
      for inq_id, fname, sid, scat, msg, st, created in cursor.fetchall():
        inquiries.append({
            "inquiry_id": inq_id,
            "full_name": fname,
            "student_id": sid,
      ...
      cursor.execute(
          "SELECT inquiry_id, full_name, student_id, service_category, message,"
          " status, created_at FROM inquiries WHERE service_category LIKE"
          " '%Hostel%' ORDER BY inquiry_id DESC"
      )
      for inq_id, fname, sid, scat, msg, st, created in cursor.fetchall():
        inquiries.append({
            "inquiry_id": inq_id,
      ...
      cursor.execute(
          "SELECT inquiry_id, full_name, student_id, service_category, message,"
          " status, created_at FROM inquiries WHERE service_category LIKE"
          " '%Ferry%' ORDER BY inquiry_id DESC"
      )
      for inq_id, fname, sid, scat, msg, st, created in cursor.fetchall():
        inquiries.append({
            "inquiry_id": inq_id,
    ...
    self.send_json({
        "success": True,
        "role": (
            "super_admin" if is_super else ("hostel" if is_hostel else "ferry")
        ),
  ...
  def handle_admin_confirm_payment(self, data):
    ...
    if btype == "hostel":
      ...
      cursor.execute(
          "UPDATE hostel_bookings SET status=?, payment_status=? WHERE"
          " booking_id=?",
      ...
      if action == "reject":
        cursor.execute(
            "UPDATE hostels SET available_beds = available_beds + 1 WHERE"
            " hostel_id = (SELECT hostel_id FROM hostel_bookings WHERE"
            " booking_id=?)",
            (bid,),
        )
    elif btype == "ferry":
      ...
      new_pay_status = "Confirmed" if action == "approve" else "Rejected"
      cursor.execute(
          "UPDATE ferry_bookings SET status=?, payment_status=? WHERE"
          " booking_id=?",
          (new_status, new_pay_status, bid),
      )
      if action == "reject":
        cursor.execute(
            "UPDATE ferries SET available_seats = available_seats + 1 WHERE"
            " ferry_id = (SELECT ferry_id FROM ferry_bookings WHERE"
            " booking_id=?)",
            (bid,),
        )
    ...
    conn.close()
    self.send_json(
        {"success": True, "message": f"Payment {action}d successfully!"}
    )
